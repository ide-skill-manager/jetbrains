...
| Subagent | `agents/<name>.agent.md` (or `.md` for backward compat) | JetBrains "Chat Agents" — registry path TBD (see [open questions](#open-questions)) |
...
- [ ] `install/AgentInstaller.kt` — stub returning `Result.failure(UnsupportedScope("agent install target pending resolution"))` until [open questions](#open-questions) close.
...
- **Custom-agent on-disk target on JetBrains.** The Mar 2026 changelog says custom agents are GA in Copilot for JetBrains and the Customizations panel exposes a registry. We don't yet know whether the registry reads `.md` files from a fixed path or persists agent definitions in IDE state (in which case Agentry would have to write to IntelliJ's `options/` XML). Resolve before Phase 3's `AgentInstaller` stops being a stub.
