...
- [x] `install/AgentInstaller.kt` — copy the source `.agent.md` to both `<project>/.github/agents/<name>.agent.md` AND `<project>/.claude/agents/<name>.agent.md` (Project), or `~/.copilot/agents/<plugin>__<name>.agent.md` AND `~/.claude/agents/<plugin>__<name>.agent.md` (Global, namespaced). Dual-write so one install reaches every tool; global filenames are namespaced because both home-level dirs are shared cross-IDE writable surfaces. Backfills `name:` and `description:` frontmatter so the loader (which rejects files with no `description`) accepts the install. See [`docs/plans/agent-installer.md`](agent-installer.md).
